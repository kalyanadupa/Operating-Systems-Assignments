/*
cc sb.c -o sb -lpthread -lm
SleepBarber <Num Customers> <Num Chairs> <rand seed>
*/
#include <stdio.h> 
#include <unistd.h> 
#include <stdlib.h> 
...
// Define the semaphores. // waitingRoom Limits the # of customers allowed // to enter the waiting room at one time. 
sem_t waitingRoom; 
...
int allDone = 0;
	...
	int main(int argc, char *argv[]) { 
		...
		int Number[MAX_CUSTOMERS]; // Check to make sure there are the right number of // command line arguments. 
		if (argc != 4) { 
		...
		RandSeed = atol(argv[3]); // Make sure the number of threads is less than the number of // customers we can support. 
		...
		srand48(RandSeed); // Initialize the numbers array. 
	...
	void *barber(void *junk) { // While there are still customers to be serviced... // Our barber is omnicient and can tell if there are // customers still on the way to his shop. 
